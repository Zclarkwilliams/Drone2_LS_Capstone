...
bool Adafruit_BNO055::begin(adafruit_bno055_opmode_t mode)
{
...
#endif
  write8(BNO055_PAGE_ID_ADDR, 0); // Force to page 0 to read chip ID
  ...
  //Delay another 20ms just for giggles, takes 7 - 19 ms to switch to config mode, but setMode already waits 30ms
  delay(20);
  ...
  uint8_t unitsel = (1 << 7) | // Orientation = Windows
  ...
  write8(BNO055_SYS_TRIGGER_ADDR, 0x0); //Clear self-test flag
  delay(10);
...
void Adafruit_BNO055::setAccConfig(void){
  write8(BNO055_PAGE_ID_ADDR, 0);
  delay(50);
  //setMode(OPERATION_MODE_CONFIG);
  write8(BNO055_PAGE_ID_ADDR, 1);
  delay(50);
  write8(BNO055_ACC_CONFIG_REG, 0x3); //Write 16G accelerometer and default of 62.5 Hz BW
  delay(50);
  write8(BNO055_PAGE_ID_ADDR, 0);
  delay(50);
  //setMode(_mode);
...
uint8_t Adafruit_BNO055::readAccConfig(void){
  //setMode(OPERATION_MODE_CONFIG);
  write8(BNO055_PAGE_ID_ADDR, 1);
  delay(50);
  uint8_t ACC_CONFIG = read8(BNO055_ACC_CONFIG_REG); //Read accelerometer config
  delay(50);
  write8(BNO055_PAGE_ID_ADDR, 0);
  //setMode(_mode);
  return ACC_CONFIG;
...
/**************************************************************************/
/*!
    @brief  Puts the chip in the default operating mode (NDOF with FMC)
*/
/**************************************************************************/
uint8_t Adafruit_BNO055::setModeDef(void)
{
  write8(BNO055_OPR_MODE_ADDR, _mode);
  delay(30);
  return _mode;
...
uint8_t Adafruit_BNO055::getMode(void)
{
  return _mode;
}
